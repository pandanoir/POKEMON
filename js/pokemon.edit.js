...
window.addEventListener("load",function(){
	...
	//					view.paint(villagers.at(i).get("job"),villagers.at(i).get("x"),villagers.at(i).get("y"));//職業ごとに違う絵柄の予定。今はテストなので下の方
					...
					mapStyle.left=-(Model.get("chara").get("x")-(screen.width/2|0))*screen.chipSize+"px";
					mapStyle.top=-(Model.get("chara").get("y")-(screen.height/2|0))*screen.chipSize+"px";
					if(Model.get("chara").get("doesRun")==0){
						switch(Model.get("chara").get("direction")){
							case "left":mapStyle.left=parseInt(mapStyle.left,10)+screen.chipSize/8*Model.get("chara").get("doesWalk")+"px";break;
							case "front":mapStyle.top=parseInt(mapStyle.top,10)-screen.chipSize/8*Model.get("chara").get("doesWalk")+"px";break;
							case "right":mapStyle.left=parseInt(mapStyle.left,10)-screen.chipSize/8*Model.get("chara").get("doesWalk")+"px";break;
							case "back":mapStyle.top=parseInt(mapStyle.top,10)+screen.chipSize/8*Model.get("chara").get("doesWalk")+"px";break;
						}
					}else{
						switch(Model.get("chara").get("direction")){
							case "left":mapStyle.left=parseInt(mapStyle.left,10)+screen.chipSize/4*(Model.get("chara").get("doesRun")%4)+"px";break;
							case "front":mapStyle.top=parseInt(mapStyle.top,10)-screen.chipSize/4*(Model.get("chara").get("doesRun")%4)+"px";break;
							case "right":mapStyle.left=parseInt(mapStyle.left,10)-screen.chipSize/4*(Model.get("chara").get("doesRun")%4)+"px";break;
							case "back":mapStyle.top=parseInt(mapStyle.top,10)+screen.chipSize/4*(Model.get("chara").get("doesRun")%4)+"px";break;
						}
					}
				...
				case "menu":
					...
					Canvas.menu.fillText(">",Model.get("cursor").get("x")+(screen.realWidth-menu.width)/2+2,Model.get("cursor").get("y")*20+(screen.realHeight-menu.height)/2+20+20);
	...
	var switchMode=(function(){
		...
		return function(scene){
			...
			switch(scene){
				...
				case "talk":
					situation="talk";
					view.show("screen").show("charaScreen").show("mes")
					break;
				case "map":
					situation="map";
...
//			document.getElementById("BGM").play()
	...
	Audio.play()*/
	view=new Display();
	document.getElementById("map").width=Model.get("world").get("width")*screen.chipSize;
	document.getElementById("map").height=Model.get("world").get("height")*screen.chipSize;
	setTimeout(function(){
